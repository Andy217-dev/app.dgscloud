...
export default function SharedCPU() {
  ...
  const { data: amd, isLoading: amdLoading } = useSelector(
    (state) => state?.Dcloud?.sharedCpuAMD
  );
  const { data: intel, isLoading: intelLoading } = useSelector(
    (state) => state?.Dcloud?.sharedCpuIntel
  );
  ...
  if (amdLoading || intelLoading) {
    return (
      <section className="py-7 px-6 sm:px-11">
        <div className="bg-glass !m-0 border-gradient w-full rounded-[10px] !mt-4 p-6 flex gap-4 flex-col min-h-[75vh] animate-pulse"></div>
      </section>
    );
  }
  ...
  return (
    <section className="py-7 px-6 sm:px-11">
      ...
      <p className="text-xl">Cloud Compute / Shared CPU</p>
            ...
            >
              <div className="flex flex-col ">
                ...
                <input
                  ...
                  maxLength={63}
                  className="w-full relative z-20 bg-white p-2 text-lg outline-none border border-[#ffffff4d] rounded-[10px] mx-[10px] text-black"
                />
              </div>
            </motion.div>
          ) : (
          ...
          hostName?.length > 0 ? (
            <>
              ...
              >
                <Button
                  variant="contained"
                  className="w-full !rounded-[12px] !bg-[#00E0FF] !text-black hover:scale-95 transition-all"
                  onClick={() => {
                    setShowSummary(true);
