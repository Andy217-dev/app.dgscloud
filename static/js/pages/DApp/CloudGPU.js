...
import Nvidia from "../../assets/image/nvidia.svg";
...
import NvidiaActive from "../../assets/image/nvidia_active.svg";
...
export default function CloudGPU() {
  ...
  const { data: cloudGpu, isLoading: cloudGpuLoading } = useSelector(
    (state) => state?.Dcloud?.cloudGPU
  );
  ...
  if (cloudGpuLoading) {
    return (
      <section className="py-7 px-6 sm:px-11">
        <div className="bg-glass w-full rounded-[10px] mt-4 p-6 flex gap-4 flex-col min-h-[75vh] animate-pulse"></div>
      </section>
    );
  }
  ...
  return (
    <section className="py-7 px-6 sm:px-11">
      ...
      <p className="text-xl">Cloud GPU</p>
      <form className="box-shadow bg-glass w-full rounded-[10px] mt-4 p-6 flex gap-4 flex-col min-h-[75vh]">
        ...
        <div className="flex gap-4 flex-col sm:flex-row items-center justify-center sm:justify-start">
          <div
            onClick={() => handleProcessor("nvidia")}
        ...
        <AnimatePresence>
          {showSummary && (
            ...
            >
              <div
                className="bg-glass flex flex-col p-4 gap-3 sm:w-auto w-[95vw] items-center border border-gradient [&>*]:relative [&>*]:z-20 rounded-[10px] max-h-[100vh] overflow-auto"
                onClick={(e) => {
                  e?.stopPropagation();
                }}
              >
